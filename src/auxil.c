...
OSQPFloat compute_rho_estimate(const OSQPSolver* solver) {
  ...
  dual_res_norm =
      c_max(dual_res_norm, temp_res_norm);  // max(||q||,||A' y||,||P x||)
...
OSQPInt adapt_rho(OSQPSolver* solver) {
  ...
  switch (settings->adaptive_rho_rl) {
...
#ifdef PRINTING
      c_eprint("Scalar policy not yet implement (but will be soon!) (%s:%d)",
               __FILE__, __LINE__);
#endif
      exitflag = 1;
      break;
    case RLQP_ADAPTIVE_RHO_VECTOR_POLICY:
      exitflag = rl_policy_compute_vec(solver->work);
      if (exitflag == 0)
        exitflag = solver->work->linsys_solver->update_rho_vec(
            solver->work->linsys_solver, solver->work->rho_vec,
            solver->settings->rho);
      break;
...
OSQPInt set_rho_vec(OSQPSolver* solver) {
  ...
  constr_types_changed = OSQPVectorf_ew_bounds_type(
      work->constr_type, work->data->l, work->data->u, OSQP_RHO_TOL,
...
OSQPInt update_rho_vec(OSQPSolver* solver) {
  ...
  if (constr_type_changed == 1) {
    exitflag = work->linsys_solver->update_rho_vec(
        work->linsys_solver, work->rho_vec, solver->settings->rho);
  }
...
static void compute_rhs(OSQPSolver* solver) {
  ...
  OSQPVectorf_add_scaled(work->xtilde_view, settings->sigma, work->x_prev, -1.,
                         work->data->q);
  ...
  if (settings->rho_is_vec) {
    ...
    OSQPVectorf_add_scaled(work->ztilde_view, -1.0, work->ztilde_view, 1.0,
                           work->z_prev);
  } else {
    OSQPVectorf_add_scaled(work->ztilde_view, 1.0, work->z_prev, -work->rho_inv,
                           work->y);
  }
}

void update_xz_tilde(OSQPSolver* solver, OSQPInt admm_iter) {
  OSQPWorkspace* work = solver->work;
...
void update_x(OSQPSolver* solver) {
  ...
  // update x
  OSQPVectorf_add_scaled(work->x, settings->alpha, work->xtilde_view,
...
void update_z(OSQPSolver* solver) {
  ...
  if (settings->rho_is_vec) {
    ...
    OSQPVectorf_add_scaled3(work->z, 1., work->z, settings->alpha,
                            work->ztilde_view, (1.0 - settings->alpha),
                            work->z_prev);
  } else {
    OSQPVectorf_add_scaled3(work->z, settings->alpha, work->ztilde_view,
...
void update_y(OSQPSolver* solver) {
  ...
  OSQPVectorf_add_scaled3(work->delta_y, settings->alpha, work->ztilde_view,
                          (1.0 - settings->alpha), work->z_prev, -1.0, work->z);
...
void compute_obj_val_dual_gap(const OSQPSolver* solver, const OSQPVectorf* x,
                              const OSQPVectorf* y, OSQPFloat* prim_obj_val,
                              OSQPFloat* dual_obj_val, OSQPFloat* duality_gap) {
  ...
  /* Compute the support function of the constraints, SC(y) = u'*max(y, 0) +
     l'*min(y, 0) by projecting y onto the polar of the recession cone of
     C=[l,u], then doing the dot products */
  OSQPVectorf_copy(work->z_prev, y);
  OSQPVectorf_project_polar_reccone(work->z_prev, work->data->l, work->data->u,
                                    OSQP_INFTY * OSQP_MIN_SCALING);

  // Round anything in the range [-OSQP_ZERO_DEADZONE, OSQP_ZERO_DEADZONE] to 0
  // to prevent very small (i.e., 1e-20) values from blowing up the numerics.
  ...
  *dual_obj_val = -0.5 * quad_term - sup_term;
  ...
  if (solver->settings->scaling) {
    ...
    *dual_obj_val *= work->scaling->cinv;
    ...
    // We always store the duality gap in the info as unscaled (since it is for
    // the user), but we keep the scaled version to use as a termination check
    // when requested.
...
static OSQPFloat compute_duality_gap_tol(const OSQPSolver* solver,
                                         OSQPFloat eps_abs, OSQPFloat eps_rel) {
...
static OSQPFloat compute_prim_res(OSQPSolver* solver, const OSQPVectorf* x,
                                  const OSQPVectorf* z) {
...
static OSQPFloat compute_prim_tol(const OSQPSolver* solver, OSQPFloat eps_abs,
                                  OSQPFloat eps_rel) {
...
static OSQPFloat compute_dual_res(OSQPSolver* solver, const OSQPVectorf* x,
                                  const OSQPVectorf* y) {
...
static OSQPFloat compute_dual_tol(const OSQPSolver* solver, OSQPFloat eps_abs,
                                  OSQPFloat eps_rel) {
  OSQPFloat max_rel_eps, temp_rel_eps;
  OSQPSettings* settings = solver->settings;
  OSQPWorkspace* work = solver->work;
  ...
  if (settings->scaling && !settings->scaled_termination) {
    // || Dinv q||
    max_rel_eps =
        OSQPVectorf_scaled_norm_inf(work->scaling->Dinv, work->data->q);
...
OSQPInt is_primal_infeasible(OSQPSolver* solver, OSQPFloat eps_prim_inf) {
  ...
  OSQPVectorf_project_polar_reccone(work->delta_y, work->data->l, work->data->u,
                                    OSQP_INFTY * OSQP_MIN_SCALING);
  ...
  if (norm_delta_y > OSQP_DIVISION_TOL) {
    ...
    if (ineq_lhs < 0.0) {
      ...
      if (settings->scaling && !settings->scaled_termination) {
        OSQPVectorf_ew_prod(work->Atdelta_y, work->Atdelta_y,
                            work->scaling->Dinv);
      }

      return OSQPVectorf_norm_inf(work->Atdelta_y) <
...
OSQPInt is_dual_infeasible(OSQPSolver* solver, OSQPFloat eps_dual_inf) {
  ...
  if (settings->scaling &&
      !settings->scaled_termination) {  // Unscale if needed
  ...
  if (norm_delta_x > OSQP_DIVISION_TOL) {
    ...
    if (OSQPVectorf_dot_prod(work->data->q, work->delta_x) < 0.0) {
      ...
      // Scale if necessary
      if (settings->scaling && !settings->scaled_termination) {
        OSQPVectorf_ew_prod(work->Pdelta_x, work->Pdelta_x,
                            work->scaling->Dinv);
      }
      ...
      if (OSQPVectorf_norm_inf(work->Pdelta_x) <
        ...
        // Scale if necessary
        if (settings->scaling && !settings->scaled_termination) {
          OSQPVectorf_ew_prod(work->Adelta_x, work->Adelta_x,
                              work->scaling->Einv);
        }
        ...
        // If you get this far, then all tests passed, so return results from
        // final test Test whether Adelta_x is in the recession cone of C = [l,
        // u]
        return OSQPVectorf_in_reccone(
            work->Adelta_x, work->data->l, work->data->u,
            OSQP_INFTY * OSQP_MIN_SCALING, eps_dual_inf * norm_delta_x);
      }
    }
  }
...
void update_info(OSQPSolver* solver, OSQPInt iter, OSQPInt polishing) {
  OSQPVectorf* x;
  OSQPVectorf* z;
...
void update_status(OSQPInfo* info, OSQPInt status_val) {
  // Update status value
...
OSQPInt check_termination(OSQPSolver* solver, OSQPInt approximate) {
  OSQPFloat eps_prim, eps_dual, eps_duality_gap, eps_prim_inf, eps_dual_inf;
  OSQPInt exitflag;
  OSQPInt prim_res_check, dual_res_check, duality_gap_check, prim_inf_check,
      dual_inf_check;
  ...
  if (work->data->m == 0) {
    prim_res_check =
        1;  // No constraints -> Primal feasibility always satisfied
  } else {
...
OSQPInt validate_data(const OSQPCscMatrix* P, const OSQPFloat* q,
                      const OSQPCscMatrix* A, const OSQPFloat* l,
                      const OSQPFloat* u, OSQPInt m, OSQPInt n) {
  OSQPInt j, ptr;
  ...
  if ((n <= 0) || (m < 0)) {
    c_eprint("n must be positive and m nonnegative; n = %i, m = %i", (int)n,
             (int)m);
    return 1;
  }
...
OSQPInt validate_settings(const OSQPSettings* settings, OSQPInt from_setup) {
  if (!settings) {
    c_eprint("Missing settings!");
    return 1;
  }

  if (from_setup && validate_linsys_solver(settings->linsys_solver)) {
    c_eprint("linsys_solver not recognized");
    return 1;
  }

  if (from_setup && settings->allocate_solution != 0 &&
  ...
  if (settings->profiler_level != 0 && settings->profiler_level != 1 &&
      settings->profiler_level != 2) {
    c_eprint("profiler_level must be either 0, 1 or 2");
    return 1;
  }

  if (settings->warm_starting != 0 && settings->warm_starting != 1) {
  ...
  if (settings->polishing != 0 && settings->polishing != 1) {
  ...
  if (from_setup && (settings->rho_is_vec != 0) &&
  ...
  if (settings->cg_tol_fraction <= 0.0 || settings->cg_tol_fraction >= 1.0) {
  ...
  if (from_setup && settings->adaptive_rho == OSQP_ADAPTIVE_RHO_UPDATE_TIME) {
    c_eprint(
        "adaptive_rho time-based adaptation requires profiling to be enabled");
    return 1;
  }
  ...
  if (settings->check_dualgap != 0 && settings->check_dualgap != 1) {
